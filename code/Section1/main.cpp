...
MatrixXd ARAP_deformation(const MatrixXd& origV, const MatrixXi& E, const SparseMatrix<double>& d0, const SparseMatrix<double>& W,
                          const vector<vector<int>>& oneRings, const VectorXi& constHandles, const MatrixXd& constPositions,
                          const int numIterations) {
    vector<Matrix3d> R(origV.rows());
    for (int i = 0; i < origV.rows(); i++) R[i] = Matrix3d::Identity();
    ...
    for (int i = 0; i < d0.cols(); i++) allVertices(i) = i;
    ...
    for (int i = 0; i < numIterations; i++) {
        // Global step: rotate each edge ij by (Ri+Rj)/2 for the previous or initial iteration's R (see Lecture notes) and try to
        // integrate to it
        for (int j = 0; j < E.rows(); j++) {
            Matrix3d REdge = (R[E(j, 0)] + R[E(j, 1)]) / 2;
            g.row(j) = (origV.row(E(j, 1)) - origV.row(E(j, 0))) * REdge;  // FIX: 0 and 1 were swapped
        }

        MatrixXd rhs = d0I.transpose() * W * (g - d0B * constPositions);  // FIX: W was missing
        MatrixXd x = solver.solve(rhs);
        for (int j = 0; j < constPositions.rows(); j++) currV.row(constHandles(j)) = constPositions.row(j);
        for (int j = 0; j < varVertices.size(); j++) currV.row(varVertices(j)) = x.row(j);

        // Local step: for existing currVertices and original positions origVertices, find the best fitting local one-ring-based
        // rotation matrices R
        for (int j = 0; j < origV.rows(); j++) {
            ...
            for (int k = 0; k < oneRings[j].size(); k++) {
                ...
                // P.row(k) = origV.row(oneRings[j][k]) - origV.row(j); // FIX: not needed
...
int main() {
    ...
    constPositions << V.row(constHandles(0)) + RowVector3d({0.01, 0.1, 0.01}), V.row(constHandles(1)) + RowVector3d({0, 0.09, -0.1}),
        V.row(constHandles(2)) + RowVector3d({0, 0.09, -0.11}), V.row(constHandles(3)), V.row(constHandles(4));
