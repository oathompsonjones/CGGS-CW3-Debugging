...
bool inline readOBJ(const std::string objFileName, directional::TriMesh& mesh) {
    Eigen::MatrixXd V;
    Eigen::MatrixXi F;
    ...
    for (int i = 0; i < vertexList.size(); i++) V.row(i) = vertexList[i];
    F.resize(faceList.size(), 3);
    for (int i = 0; i < faceList.size(); i++) F.row(i) = faceList[i];
